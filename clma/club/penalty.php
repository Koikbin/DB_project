...
<?php include('../nav/nav_leader.php');?>
<div class="container">
    ...
    <?php
        function search_point($semester ,$club_name_ch)
        {
            global $pdo;
            if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $sql_search_point = "Select v.sum(*)
                                        From VIOLATE as v
                                        Where v.semester = :semester and v.club_id = :club_id;";
        
        
                try {
                    $statement = $pdo->prepare($sql_point);
                    $statement->bindParam(':club_id', $club_id);
                    $statement->bindParam(':semester', $semester);
                    $statement->execute();
                    $result_point_sum = $statement->fetchAll(PDO::FETCH_ASSOC);
                }catch (PDOException $e) {
                ...
                if ($result_point_sum) {
                    if ($result_point_sum->num_rows > 0) {
                        $point = $result_point_sum['0']['sum']; // 不確定
                        }
                }
            }
            return $point;
