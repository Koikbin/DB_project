...
<div class="container">
    ...
    <p>所有社團活動：</p>
    <?php
        function venue_admin_all_activity_query($venueAdminId) {
            global $pdo;
            // echo "get id: " . $venueAdminId . "<br>";
            try {
                // Start a transaction
                ...
                $query = "
                    WITH venue_ AS (
                        SELECT vn.building_name
                        FROM venue AS vn
                        WHERE vn.\"venue_admin_ID\" = :venueAdminId
                    )
                    SELECT
                        act.sn,
                        act.\"club_ID\",
                    ...
                    WHERE
                        uts.building_name IN (SELECT * FROM venue_)
                    GROUP BY
                        act.sn
                    ORDER BY
                        act.sn
                ...
                $statement->bindParam(':venueAdminId', $venueAdminId, PDO::PARAM_STR);
                $statement->execute();
        ...
        $venueAdminId = $ID;
        // Example usage
        $results = venue_admin_all_activity_query($venueAdminId);
